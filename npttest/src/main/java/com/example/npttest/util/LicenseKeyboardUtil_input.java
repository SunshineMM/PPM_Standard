...
public class LicenseKeyboardUtil_input {
    ...
    public static int currentEditText_input = 0;//默认当前光标在第一个EditText
    private String license = "";
    public static int etsize=6;
    public LicenseKeyboardUtil_input(Context ctx, EditText edits[]) {
        this.ctx = ctx;
        this.edits = edits;
        ...
        public void onKey(int primaryCode, int[] keyCodes) {
            ...
            if(primaryCode == 112){ //xml中定义的删除键值为112
                if (currentEditText_input!=0){
                    edits[currentEditText_input].setText("");//将当前EditText置为""并currentEditText-1
                    //edits[currentEditText].setBackgroundColor(Color.parseColor("#ffffff"));
                    currentEditText_input--;
                }

                if(currentEditText_input < 1){
                    //切换为省份简称键盘
                    keyboardView.setKeyboard(k1);
                }
                if(currentEditText_input < 0){
                ...
                initcolor();
                edits[currentEditText_input].setBackgroundResource(R.drawable.keyboard_bg_red);
                ...
                if (currentEditText_input == 0) {
                    ...
                    //第二位必须大写字母
                    if(currentEditText_input == 1 && !letterAndDigit[primaryCode].matches("[A-Z]{1}")){
                        return ;
                    }
                    edits[currentEditText_input].setText(letterAndDigit[primaryCode]);
                    //edits[currentEditText].setBackgroundColor(Color.parseColor("#1CB9F0"));
                    currentEditText_input++;
                    if (currentEditText_input > etsize) {
                        currentEditText_input = etsize;
                    }
                    initcolor();
                    edits[currentEditText_input].setBackgroundResource(R.drawable.keyboard_bg_red);
                }
